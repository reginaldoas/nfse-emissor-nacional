...
class Danfse
{
    ...
    /** @var float */
    protected $margsup = 1.5;

    /** @var float */
    protected $margesq = 1.5;

    /** @var float */
    protected $marginf = 1.5;

    /** @var float */
    protected $wPrint = 207;

    /** @var float */
    protected $hPrint = 294;
    ...
    {
        $this->creditos = trim((string) $message);
    ...
    private function drawHeader($logo = '')
    {
        ...
        $this->text(self::COL4, 3.5, self::CELL, 3.4, $this->dash($municipio), $this->fontContent(8));
        $this->text(self::COL4, 8.7, self::CELL, 2.7, 'Ambiente Gerador: ' . $this->dash($this->value('ambGer', $this->infNFSe)), $this->fontContent(6));
        $this->text(self::COL4, 11.3, self::CELL, 2.7, 'Tipo de Ambiente: ' . $this->dash($this->value('tpAmb', $this->infDPS)), $this->fontContent(6));
    }
    ...
    {
        return dirname(__DIR__, 2) . '/assets/logo-nfse.jpg';
    }
    ...
    private function drawServico($y)
    {
        ...
        $serv_nfse = $this->childNode('serv', $this->infNFSe);
        $cServ = $this->childNode('cServ', $serv);
        $loc = $this->childNode('locPrest', $serv);
        $codigos = $this->joinNonEmpty([$this->formatField($this->value('cTribNac', $cServ), '##.##.##'), $this->value('cTribMun', $cServ)], ' / ');
        $descCodigo = $this->value('xTribNac', $serv_nfse) ?: $this->value('xTribMun', $serv_nfse);
    ...
    private function drawIssqn($y, ?DOMElement $prest)
    {
        ...
        $this->drawField('Município / Sigla UF / País da Incidência do ISSQN', $this->issqnLocal($this->infNFSe), self::COL3, $y, self::CELL2, self::ROW);
        ...
        $this->drawMoneyField('BC ISSQN', $this->value('vBC', $valores), self::X, $y + 6.5, self::CELL, self::ROW);
        $this->drawField('Alíquota Aplicada', $this->percent($this->value('pAliqAplic', $valores)), self::COL2, $y + 6.5, self::CELL, self::ROW);
        $this->drawField('Retenção do ISSQN', $this->tpRetIssqn($this->value('tpRetISSQN', $tribMun)), self::COL3, $y + 6.5, self::CELL, self::ROW);
        $this->drawMoneyField('ISSQN Apurado', $this->value('vISSQN', $valores), self::COL4, $y + 6.5, self::CELL, self::ROW);
    ...
    private function drawFooter()
    {
        ...
        $this->text(self::X, $y, self::WIDTH, 2.5, $this->powered ? 'Powered by reginaldoas/nfse' : '', $font, 'T', 'R', false);
    }
    ...
    {
        $nac = $this->firstNode('enderNac', $end) ?: $this->firstNode('endNac', $end);
        $ext = $this->firstNode('endExt', $end);
        $base = $nac ?: $ext;

        $codigo_municipio = (int)($this->firstValue(['xMun', 'xCidade', 'cMun'], $base));
        if (!empty($codigo_municipio)) {
            $municipio = Tools::getCidadeIbge($codigo_municipio);
    ...
    private function ibgeCep(?DOMElement $end)
    {
        ...
        return $this->dash($this->joinNonEmpty([
            $this->formatField($this->value('cMun', $base), '##.#####'),
    ...
    {
        $codigo_local = $this->firstValue(['cLocPrestacao'], $loc);
        if (!empty($codigo_local)) {
            $municipio = Tools::getCidadeIbge($codigo_local);
            return $this->dash($this->joinNonEmpty([
                $municipio['nome'],
                $municipio['sigla'],
                "-"
    ...
    private function issqnLocal(?DOMElement $infNFSe)
    {
        $codigo_local = $this->value('cLocIncid', $infNFSe);
        if (!empty($codigo_local)) {
            $municipio = Tools::getCidadeIbge($codigo_local);
            return $this->dash($this->joinNonEmpty([
                $municipio['nome'],
            ...
            return $this->dash($this->joinNonEmpty([
                $this->firstValue(['xLocIncid', 'xLocIncid', 'cLocIncid'], $infNFSe),
                $this->value('UF', $locIncidencia),
                $this->value('cPaisIncid', $locIncidencia)
            ], ' / '));
        }
    }
    ...
    private function sumValues(array $values)
    {
        ...
        return $found ? number_format($sum, 2, '.', '') : '0,00';
    }
    ...
    private function tpRetPisCofins($value)
    {
        ...
        return isset($map[$value]) ? $this->dash($this->joinNonEmpty([$value, $map[$value]], ' - ')) : $this->dash($value);
    }
    ...
    protected function formatField($campo = '', $mascara = '')
    {
        ...
        $limpo = preg_replace('/\D/', '', (string) $campo);
        $resultado = '';
        $i = 0;
        $len = strlen($limpo);
        for ($m = 0, $mlen = strlen($mascara); $m < $mlen; $m++) {
            if ($mascara[$m] === '#') {
                if ($i >= $len) {
                    break;
                }
